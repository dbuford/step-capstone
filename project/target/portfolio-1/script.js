...
function loadEdit(){
    fetch('/data').then(response => response.json()).then((entries) => {
          ...
          if(entry.email== localStorage.getItem("userEmail")){
              ...
              var x=document.getElementById("locationid").options;
              var y=document.getElementById("raceid").options;
              var z=document.getElementById("genderid").options;
              var r=document.getElementById("majorid").options;
              var l=document.getElementById("gradeid").options;
              var w=document.getElementById("incomeid").options;
              entry.race = y.value;
              entry.gender = z.value;
              entry.income = w.value;
              entry.major = r.value;
              entry.grade = l.value;
              entry.state = x.value;

              if(entry.race.includes(el.value)){
                el.checked=true;
            }
        }
